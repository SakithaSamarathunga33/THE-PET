...
const PetManagement = ({ onDataChange }) => {
  ...
  const validateForm = () => {
    ...
    if (!formData.breed.trim()) {
      ...
      errors.breed = 'Breed cannot exceed 50 characters';
    }
    ...
    if (!formData.age) {
      ...
      errors.age = 'Age seems unrealistic';
    }
    ...
    } else if (formData.type === 'Dog' && Number(formData.weight) > 100) {
      errors.weight = 'Dog weight seems unrealistic';
    } else if (formData.type === 'Cat' && Number(formData.weight) > 25) {
      errors.weight = 'Cat weight seems unrealistic';
    } else if (formData.type === 'Bird' && Number(formData.weight) > 15) {
      errors.weight = 'Bird weight seems unrealistic';
    } else if (formData.type === 'Fish' && Number(formData.weight) > 10) {
      errors.weight = 'Fish weight seems unrealistic';
    } else if (formData.type === 'Rabbit' && Number(formData.weight) > 10) {
      errors.weight = 'Rabbit weight seems unrealistic';
    }
    ...
    if (!formData.price) {
      ...
      errors.price = 'Price cannot exceed 1,000,000 Rs.';
    }
    ...
    // Image URL validation if using custom image
    if (useCustomImage && formData.imageUrl) {
      try {
        new URL(formData.imageUrl);
        if (!formData.imageUrl.match(/\.(jpeg|jpg|gif|png|webp)$/i)) {
          errors.imageUrl = 'URL must point to an image file (JPEG, PNG, GIF, WEBP)';
        }
      } catch (e) {
  ...
  const toggleUseCustomImage = () => {
    if (useCustomImage) {
      ...
      // Clear image URL validation error
      if (validationErrors.imageUrl) {
        setValidationErrors(prev => ({...prev, imageUrl: null}));
      }
    } else {
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  ...
                  {validationErrors.medicalHistory && (
                    <p className="mt-1 text-sm text-red-600">{validationErrors.medicalHistory}</p>
                  )}
                  <p className="mt-1 text-xs text-gray-500">
                    {formData.medicalHistory.length}/1000 characters
                  </p>
                </div>
